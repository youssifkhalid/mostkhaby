...
import {
  ...
  Check, Phone, Video, Mic, Image as ImageIcon, Eraser, CheckSquare,
  Edit3, Reply, Copy, CheckCheck, Clock, Smile,
  Search, ChevronDown, Play, Pause, ZoomIn
} from "lucide-react";
import { useNavigate, useParams } from "react-router-dom";
import { useChatMessages, useTypingIndicator } from "@/hooks/useChats";
import { useChatReactions, useChatMessageActions } from "@/hooks/useChatActions";
...
function groupMessages(msgs: Msg[]) {
  const groups: { date: string; rawDate: string; msgs: Msg[] }[] = [];
  msgs.forEach((m) => {
    const d = format(new Date(m.created_at), "d MMMM yyyy", { locale: ar });
    const last = groups[groups.length - 1];
    if (last && last.date === d) last.msgs.push(m);
    // ✅ بنحفظ raw date عشان dateLabel يشتغل صح
    else groups.push({ date: d, rawDate: m.created_at, msgs: [m] });
  });
  return groups;
}
...
function isYesterday(iso: string) {
  const d = new Date(iso); const y = new Date();
  y.setDate(y.getDate() - 1);
  return d.toDateString() === y.toDateString();
}

// ✅ بياخد rawDate صح — كان بيتنادى بـ argument واحد بس وده bug
function dateLabel(displayDate: string, rawDate: string) {
  if (isToday(rawDate)) return "اليوم";
  if (isYesterday(rawDate)) return "أمس";
  return displayDate;
...
const Bubble = memo(({
  ...
  return (
    // ✅ إصلاح اتجاه الفقاعات:
    // رسائل المستخدم (isMine) على اليمين، رسائل الآخر على اليسار
    <div className={`flex ${isMine ? "justify-end" : "justify-start"} ${!prevSameSender ? "mt-3" : "mt-0.5"} px-3
      ${selected ? "bg-primary/10 -mx-3 px-6 py-0.5 rounded-lg" : ""}`}
      onClick={selectionMode ? onClick : undefined}>

      {selectionMode && (
        <div className={`flex items-center ${isMine ? "mr-2" : "ml-2"} flex-shrink-0`}>
          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors
            ${selected ? "bg-primary border-primary" : "border-muted-foreground/40"}`}>
            {selected && <Check size={11} className="text-primary-foreground" />}
          </div>
...
const ChatPage = () => {
  ...
  // ✅ startCall محمية بـ try/catch — لو جدول calls مش موجود ما يكرشش
  const startCall = useCallback(async (type: "audio" | "video") => {
    if (!user?.id || !otherUserId || !chatId) return;
    try {
      const { data, error } = await supabase.from("calls")
      ...
      <div ref={scrollRef} onScroll={handleScroll}
        ...
        <div className="max-w-2xl mx-auto">
          {isLoading ? (
            ...
            grouped.map((group) => (
              <div key={group.date}>
                {/* ✅ dateLabel بياخد rawDate صح دلوقتي */}
                <div className="flex items-center gap-3 px-4 py-3">
                  <div className="flex-1 h-px bg-border/20" />
                  <span className="text-[11px] text-muted-foreground/60 font-cairo bg-secondary/30 px-3 py-1 rounded-full">
                    {dateLabel(group.date, group.rawDate)}
                  </span>
                  <div className="flex-1 h-px bg-border/20" />
                </div>
